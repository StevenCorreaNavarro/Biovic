...
<body>
  ...
  // -------- PDF (formato IPS) mejorado: footer con fecha y paginación --------
  async function exportPdfIPS(dt) {
    try {
      ...
      // Fecha/hora de generación (locale ES)
      let generado = '';
      try {
        generado = new Intl.DateTimeFormat('es-CO', { dateStyle:'medium', timeStyle:'short' }).format(new Date());
      } catch(e) {
        generado = new Date().toLocaleString(); // fallback
      }

      const doc = {
        info: {
          title: 'Inventario Físico',
          author: '{{ Auth::user()->name ?? "Sistema" }}',
          subject: 'Listado de equipos biomédicos',
          keywords: 'inventario, equipos, IPS, mantenimiento'
        },
        pageSize: 'A4',
        pageOrientation: 'landscape',
        pageMargins: [20,20,20,28], // espacio extra para footer
        defaultStyle: { fontSize: 9 },
        styles: { tableHeader: { bold:true, fontSize:10, color:'#111', fillColor:'#F2F2F2', alignment:'center' } },
        footer: function(currentPage, pageCount) {
          return {
            columns: [
              { text: 'Generado: ' + generado, alignment: 'left', margin: [20, 0, 0, 0] },
